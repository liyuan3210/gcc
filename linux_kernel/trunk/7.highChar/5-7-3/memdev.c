...
extern void create_devices(int i);
extern void delete_devices();
/*
 * 文件打开函数
 * */
...
//创建设备节点
void create_devices(int i){
	/*在自动创建设备节点之前，先创建sysfs下对应的类(第一次mem_class为NULL时执行)*/
	if(mem_class==NULL)mem_class=class_create(THIS_MODULE, "mem_class");
	/*在sysfs中注册该设备，这将引起udev/mdev创建对应的设备节点*/
